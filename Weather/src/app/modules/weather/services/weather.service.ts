...
export class WeatherService {
  ...
  public GetWeather(payload: WeatherPayload): Observable<WeatherInterface> {
    let url: string = `https://api.openweathermap.org/data/2.5/weather?lat=${
      payload.lat
    }&lon=${payload.lon}&lang=${payload.lang ? payload.lang : ''}&mode=${
      payload.mode ? payload.mode : ''
    }&units=${payload.units ? payload.units : ''}&appid=${this.apiKey}`;
